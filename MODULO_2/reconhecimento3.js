...
function preload() {
  bkgImg = loadImage('../RECURSOS/IMAGENS/mod2-rec3.png');
...
function setup() {
  ...
  posInput = createVector((innerWidth / 12) * 2.8, (innerHeight / 15) * 7); //altura das caixas
  ...
  btProxImgVetor = createVector((innerWidth / 12) * 10.6, (innerHeight / 10) * 2);
  btSomImgVetor = createVector((innerWidth / 70) * 10.6, (innerHeight / 10) * 2);
  btVoltarImgVetor = createVector((innerWidth / 13) * 11, (innerHeight / 10) * 2.7);
...
function draw() {
  ...
  if (blocos[blocoAtual].nSilabas == 3) {
    ...
    button.position(input2.x + 0.5 * input1.width - 99, posInput.y + input2.height + 30);
  } else {
    input1.position(posInput.x + 120, posInput.y);
    input2.position(input1.x + input1.width + 5, posInput.y);
    input3.position(-200, -200);
    button.position(input1.x + 0.5 * input1.width + 35, posInput.y + input2.height + 10);
  }
...
class Bloco {
  ...
  constructor(silabas, palavra) {
    ...
    this.posSilaba = createVector(42 * (innerWidth / 80), 20 * (innerHeight / 54)); // silaba de cima
